<template>
	<view class="control-page">
		...
		>
			<view class="content-inner">
				...
				<CurrentStatusCard
					...
					:walk-speed-kmh="walkSpeedKmh"
					:command-loading="store.commandLoading"
					:on-emergency-stop="showEmergencyStopConfirm"
					:on-clear-fault="handleClearFault"
					:on-calibrate="hasFault ? null : showCalibrateDialog"
				/>
				...
				<view class="dashboard-wrap">
					...
					<image
						class="exo-image"
						src="/static/exo_view2.png"
						mode="aspectFit"
					/>

					<!-- 指标气泡 -->
					<MetricBubble
						class="bubble-top-left"
						:value="String(store.status.stepCount)"
						label="步数"
						:accent="selectedMode.accent"
						:progress="(store.status.stepCount / 10000).clamp(0, 1)"
						:animated="true"
					/>
					<MetricBubble
						...
						label="使用时长"
						:accent="selectedMode.accent"
						:progress="(store.status.runtimeSeconds / 7200).clamp(0, 1)"
						:animated="true"
					/>
					<MetricBubble
						class="bubble-bottom-left"
						:value="bottomLeftValue"
						:label="bottomLeftLabel"
						:accent="selectedMode.accent"
						:progress="bottomLeftProgress"
						:animated="true"
					/>
					<MetricBubble
						class="bubble-bottom-right"
						:value="String(store.status.battery) + '%'"
						label="电量"
						:accent="selectedMode.accent"
						:progress="(store.status.battery / 100).clamp(0, 1)"
						:animated="true"
					/>
				...
				<!-- 等级选择器 -->
				<LevelSelector
					:visible="store.mode !== 'transparent'"
					:label="store.mode === 'training' ? '阻抗等级' : '助力等级'"
					:level="activeLevel"
					:accent="selectedMode.accent"
		...
		<view class="bottom-bar">
			<ModeSwitchBar
				:modes="modes"
				:selected-mode-id="store.mode"
				:loading-target-mode="loadingTargetMode"
...
import {
	startRunning, stopRunning, switchMode, changeLevel, clearFault,
...
async function onModeSelect(mode) {
	...
	if (store.enabled) {
		uni.showToast({ title: '请先停止运行再切换模式', icon: 'none', duration: 2000 });
		return;
	}
...
function showCalibrateDialog() {
	uni.showModal({
		...
		success: async (res) => {
			if (res.confirm) {
				const { sendCommand } = await import('../../services/exo-command.js');
				const { CMD } = await import('../../utils/protocol.js');
				try {
					await sendCommand(store.deviceId, CMD.calibrateZero);
					uni.showToast({ title: '校准已启动', icon: 'success' });
				} catch (e) {
					uni.showToast({ title: e.message || '校准失败', icon: 'none' });
				}
			}
		},
	});
...
function handleDebugTap() {
	...
	if (debugTapCount >= 5) {
		...
		uni.showToast({ title: '调试模式已激活', icon: 'none' });
	}
}
