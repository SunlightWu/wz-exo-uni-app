<template>
  <view
    ...
    :style="[radioStyle, elShape == 'button' && iconWrapStyle]"
    :class="[radioClasses, elShape == 'button' && iconClasses]"
  >
    <view
      v-if="elIcon"
    ...
    <view class="u-radio__label-wrap" @tap.stop="labelClickHandler">
      <slot>
        <text
          class="u-radio__text"
          :style="{
 ...
 * @tutorial https://uview.d3u.cn/components/radio.html
 * @property {String | Number}	name			radio的名称
 * @property {String}			shape			形状，square为方形，circle为圆型, button为按钮
 ...
 * @property {String}			activeLabelColor	选中时的label颜色
...
export default {
  ...
  computed: {
    ...
    iconClasses() {
      ...
      if (this.elDisabled) {
        classes.push("u-radio__" + this.elShape + "--disabled");
      }
      if (this.checked && this.elDisabled) {
        classes.push("u-radio__" + this.elShape + "--disabled--checked");
      }
    ...
    iconWrapStyle() {
      ...
      style.backgroundColor =
        this.checked && !this.elDisabled
          ? this.elActiveColor
          : this.plain === false || this.parentData.plain === false
          ? this.elInactiveColor
          : "#ffffff";

      style.borderColor =
        this.checked && !this.elDisabled ? this.elActiveColor : this.elInactiveColor;
      ...
      // 设置按钮内容的对齐方式
      if (this.elShape === "button") {
        style.justifyContent = this.elAlign;
      }
    ...
    radioClasses() {
      ...
      if (this.parentData.iconPlacement) {
        classes.push(`u-radio-label--${this.parentData.iconPlacement}`);
      }
  ...
  methods: {
    ...
    wrapperClickHandler(e) {
      // 如果是按钮形状，点击整个按钮都应该触发选中事件
      if (this.elShape === "button") {
        this.preventEvent(e);
        if (!this.elDisabled) {
          this.setRadioCheckedStatus();
        }
      } else if (this.parentData.iconPlacement === "right") {
    ...
    setRadioCheckedStatus() {
      ...
      if (this.$u.test.func(this.parent.unCheckedOther)) {
...
.u-radio {
  ...
  &__icon-wrap {
    ...
    &--button {
      padding: $u-radio-button-padding;
      border-width: $u-radio-wrap-border-width;
      border-color: $u-radio-wrap-border-color;
      border-style: solid;
      border-radius: $u-radio-button-border-radius;
    }
  }
