...
1. 合并issues
## 3.7.13（2025-12-03）
1. navbar组件新增customStyle属性，可实现自定义背景图
## 3.7.12（2025-12-01）
1. 修复`action-sheet`组件滚动条无效的问题
2. 修复`modal`组件 loading 状态下仍然可以点击的问题
3. 修复`datetime-picker`组件条件编译报错的问题
## 3.7.11（2025-11-07）
修改cropper组件旋转动画
## 3.7.10（2025-11-06）
1. 新增 noticeBar 组件 iconColor 属性
## 3.7.9（2025-11-03）
优化一些细节
## 3.7.8（2025-11-01）
优化示例
...
优化示例
## 3.7.6（2025-10-30）
1. 修复`upload`组件multiple=false的问题
2. 新增`cropper`组件zIndex属性，用于调整层级
## 3.7.5（2025-10-28）
1. 修复`table`组件新增selectedRowBgColor属性
## 3.7.4（2025-10-21）
1. 修复`amount`组件值不能为0的问题
2. 修复`action-sheet`组件第一次点击正常再次点击不弹出的问题
## 3.7.3（2025-10-10）
1. 修复`calendar`组件confirm返回空的问题
2. 修复`upload`组件header无效的问题
## 3.7.2（2025-10-08）
1. 修复`upload`组件问题
## 3.7.1（2025-09-30）
1. 修复`grid`组件问题
...
1. 修复`paging`组件问题
## 3.6.37（2025-09-26）
1. 修复`notice-bar`组件问题
## 3.6.36（2025-09-26）
1. 优化多语言
## 3.6.35（2025-09-25）
1. 新增`input`组件`inputmode`属性
## 3.6.34（2025-09-25）
1. 修复`search`组件清除按钮问题
## 3.6.33（2025-09-23）
1. 修复自定义icon图标
## 3.6.32（2025-09-22）
1. 优化`calendar`组件，减少计算量，提升性能
2. 优化`upload`组件，修复h5端开启图片压缩后，后端无法获取原始文件名的问题
## 3.6.31（2025-09-20）
1. 优化文档
## 3.6.30（2025-09-19）
1. 修复一些已知的问题
...
2. 修复`tabs`组件手势滑动问题
3. 修复`actionSheet`组件问题
## 3.6.23（2025-09-13）
1. 修复与优化`dropdown` 组件
2. 修复`cell` 组件label的内容显示在同一行
3. 修复`slider` 组件垂直滑块不居中
## 3.6.22（2025-09-12）
1. 新增 `input-tag` 组件
## 3.6.21（2025-09-12）
1. 优化 `tag` 组件
## 3.6.20（2025-09-11）
1. 新增 `getLocation` API方法
## 3.6.19（2025-09-10）
1. 优化示例
## 3.6.18（2025-09-10）
1. 优化`upload`组件图标
...
2. 优化`tree`组件图标
## 3.6.14（2025-09-08）
1. 新增`tabbar`组件`url`,`linkType`属性，自动识别高亮，无效手动管理状态
2. 新增`tag`组件`customStyle`属性
3. 修复`slider`组件无法拖拽到最小值以及弹窗中位置错乱的问题
4. 修复`select`组件value不能为0的问题
## 3.6.13（2025-09-07）
1. 重构`dropdown`组件，提供更多功能，可以满足各种复杂的界面设计需求
2. 新增4个`icon`图标
## 3.6.12（2025-09-07）
1. 新增`sliderButton`滑动按钮组件
2. 修复`datetimePicker`,`areaPicker`组件input属性问题
## 3.6.11（2025-09-06）
1. 修复一些组件vue2小程序警告
## 3.6.10（2025-09-05）
1. 修复`numberbox`组件错误
